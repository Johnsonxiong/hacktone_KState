package com.example.fubin.gmt;

import android.content.Intent;
import android.renderscript.ScriptGroup;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.TextView;

import java.io.InputStream;
import java.util.Random;
import java.util.Scanner;

public class truthActivity extends AppCompatActivity {

    String rs;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_truth);

        if(getIntent().hasExtra("my thought"))
        {
            String text = getIntent().getExtras().getString("my thought");

            rs = text;
        }

        Random r = new Random();

        int tru = r.nextInt(151) + 1;

        /*InputStream input = getResources().openRawResource(R.raw.truth);*/

        InputStream input = getResources().openRawResource(getResources().getIdentifier("truth","raw",getPackageName()));

        Scanner s = new Scanner(input);

        int count = 0;

        while(s.hasNext())
        {
            String Line = s.nextLine();
            if(count == tru)
            {
                TextView tv1 = (TextView) findViewById(R.id.randomtruth);
                tv1.setText(Line);
            }
            count+=1;
        }

        Button change = (Button) findViewById(R.id.change);
        change.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                InputStream input = getResources().openRawResource(getResources().getIdentifier("truth","raw",getPackageName()));

                Random rn = new Random();

                int newnum = rn.nextInt(151) + 1;

                Scanner s = new Scanner(input);

                int count = 0;

                while(s.hasNext())
                {
                    String Line = s.nextLine();
                    if(count == newnum)
                    {
                        TextView tv1 = (TextView) findViewById(R.id.randomtruth);
                        tv1.setText(Line);
                    }
                    count+=1;
                }
            }
        });

        Button Yes = (Button)findViewById(R.id.back);
        Yes.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent startIntent = new Intent(getApplicationContext(), thirdactivity.class);
                startIntent.putExtra("my thought", rs);
                startActivity(startIntent);
            }
        });

        Button No = (Button)findViewById(R.id.finish);
        No.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent startIntent = new Intent(getApplicationContext(), fourthActivity.class);
                startActivity(startIntent);
            }
        });


    }
}
